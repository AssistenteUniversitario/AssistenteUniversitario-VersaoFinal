...
ListaAulas *removeAula (ListaAulas *aula, int aulaEscolhida)
{
    ...
    {
        int i = 0;
        ListaAulas *aux = aula, *auxAntes = aula;
        for (i = 1; i < aulaEscolhida; i++)
        {
            auxAntes = aux;
            aux = aux->prox;
        }
    ...
    do
    {
        ...
        switch(opcao)
        {
            ...
            case 3:
                ...
                if (cont >= 1)
                {
                    while (aulaEscolhida < 1 || aulaEscolhida-1 > cont)
                    {
                        printf("\n\nDigite uma aula valida: ");
                        scanf("%d", &aulaEscolhida);
                    }
                    aulas = removeAula(aulas, aulaEscolhida);
                }else{
                    printf("Nao ha aulas cadastradas\n\n");
                    system("pause");
                }
...
void gravaAula(ListaMaterias *materia, char *nome, int numPeriodo)
{
    ...
    {
        cont++;
        sprintf(buffer1, "Assistente Universitario\\Data\\%s.%d.%dAu%d", nome, numPeriodo, materia->numero, cont);
        arquivo = fopen(buffer1, "wb");
        fwrite(aux, sizeof(ListaAulas), 1, arquivo);
        fclose(arquivo);
        aux = aux->prox;
    }
    sprintf(buffer2, "Assistente Universitario\\Data\\%s.%d.%dAu.txt", nome, numPeriodo, materia->numero);
...
ListaAulas *carregaAula (ListaMaterias *materia, char *nome, int numPeriodo)
{
    ...
    char buffer1[150], buffer2[150];
    int cont = 0, i;
    sprintf(buffer1, "Assistente Universitario\\Data\\%s.%d.%dAu.txt", nome, numPeriodo, materia->numero);
    ...
    {
        aux = NULL;
        sprintf(buffer2, "Assistente Universitario\\Data\\%s.%d.%dAu%d", nome, numPeriodo, materia->numero, i);
