            ...
            case 3:
                ...
                while (aux != NULL)
                {
                    if (aux->numero == periodoEscolhido)
                    {
                        ...
                        imprimeMateriasTexto(aux->materias, aux, aluno);
                        achou = 1;
                        gravaMateria(aux, aluno->nome);
                        aux->materias = esvaziaMateria(aux->materias);
                    }
                    aux = aux->prox;
                }
                if (achou == 0)
...
void gravaPeriodo(Aluno *aluno)
{
    ...
    {
        cont++;
        sprintf(buffer1, "Assistente Universitario\\Data\\%s.%d", aluno->nome, cont);
        arquivo = fopen(buffer1, "wb");
        fwrite(aux, sizeof(PilhaPeriodos), 1, arquivo);
        fclose(arquivo);
        aux = aux->prox;
    }
    sprintf(buffer2, "Assistente Universitario\\Data\\%s.Num.txt", aluno->nome);
...
PilhaPeriodos *carregaPeriodo (Aluno *aluno)
{
    ...
    char buffer1[150], buffer2[150];
    int cont = 0, i;
    sprintf(buffer1, "Assistente Universitario\\Data\\%s.Num.txt", aluno->nome);
    ...
    {
        aux = NULL;
        sprintf(buffer2, "Assistente Universitario\\Data\\%s.%d", aluno->nome, i);
